...
describe.sequential('POST /api/exchange', () => {
  ...
  it('名刺交換ができる（QR有効）', { timeout: 30000 }, async () => {
    ...
    // user B が QR を使って交換
    const exRes = await post('/api/exchange', { id: idB, password_hash: passB, qr });
    expect(exRes.status).toBe(200);
    expect(exRes.data?.success).toBe(true);

    // 両者の friends にお互いがいること
    const accA = await get(`/api/account?id=${encodeURIComponent(idA)}&password_hash=${encodeURIComponent(passA)}`);
    ...
    const accB = await get(`/api/account?id=${encodeURIComponent(idB)}&password_hash=${encodeURIComponent(passB)}`);
